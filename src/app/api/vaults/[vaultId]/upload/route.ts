...
/**
 * POST /api/vaults/[vaultId]/upload - Get presigned upload URL and save to local DB
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ vaultId: string }> }) {
  try {
    ...
    try {
      await db.insert(vaultObjects).values({
        id: data.objectId,
        vaultId,
        filename,
        contentType,
      ...
      console.log(`Saved object ${data.objectId} to local DB`);
    } catch (dbError) {
